...
export default async function AnalyticsPage() {
  ...
  const performanceData = models
    .map((model) => {
      const completedVideos = model.videos.filter((v) => v.status === "COMPLETED");
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Comprehensive performance metrics and statistics across all AI video generation models
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="border rounded-lg p-4">
            ...
            <div className="text-2xl font-bold">{performanceData.reduce((sum, d) => sum + d.totalVideos, 0)}</div>
          </div>
          <div className="border rounded-lg p-4">
            <div className="text-sm text-muted-foreground">Completed</div>
            <div className="text-2xl font-bold">{performanceData.reduce((sum, d) => sum + d.completedVideos, 0)}</div>
          </div>
          <div className="border rounded-lg p-4">
            <div className="text-sm text-muted-foreground">Total Votes</div>
            <div className="text-2xl font-bold">{performanceData.reduce((sum, d) => sum + d.totalVotes, 0)}</div>
          </div>
        </div>
        ...
        {performanceData.length > 0 ? (
          ...
          <div className="border rounded-lg p-12 text-center">
            <p className="text-muted-foreground">
              No performance data available yet. Generate some comparisons to see analytics!
            </p>
          </div>
        )}
      </div>
